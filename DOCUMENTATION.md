...
Централизованный браузер всех созданных материалов. Состоит из семи разделов (переключение кнопками).

**Поиск** — строка «Поиск…» в панели инструментов фильтрует список активного раздела по имени файла/проекта в реальном времени.
...
#### Шаблоны субтитров
- Список сохранённых пресетов стиля субтитров (вкладка Видео)
- Визуальный предпросмотр и JSON-параметры
- Применить шаблон в редакторе видео

#### Проекты (Image Video Editor)
- Список проектов редактора (количество клипов, общая длительность)
- Открыть в редакторе, скачать как `.project`, переименовать, удалить

#### Видео шаблоны (Image Video Editor)
- Список шаблонов проектов редактора
- Редактировать в редакторе, дублировать, переименовать, удалить

#### Логи
- Список серверных лог-файлов
...
| Параметр | Описание |
|---|---|
| Включить | Чекбокс «Подсветка» |
| Цвет подсветки | HEX (по умолчанию #ffdd00) |
| Режим | `word` — только текущее слово; `cumulative` — все предыдущие слова |
| По буквам | Текущее слово печатается побуквенно в течение его временно́го слота |
| Фон-маркер | За текущим/активным словом появляется цветной фоновый прямоугольник (имитация маркера) |
| Только слово | Все остальные слова скрыты (невидимы), но занимают место в строке — положение не скачет |
| Зум слова | Активное слово отображается крупнее (150% масштаб) |

Все четыре эффекта совместимы между собой и могут быть включены одновременно.  
Режим подсветки цветом (`word` / `cumulative`) действует независимо от остальных эффектов.
...
| Параметр | Описание |
|---|---|
| Включить | Чекбокс «Karaoke» |
| Режим | `word` (только текущее слово) / `cumulative` (накопительно) |
| Цвет подсветки | HEX (по умолчанию #ffdd00) |
| По буквам | Текущее слово печатается побуквенно в течение его временно́го слота |
| Фон-маркер | Цветной фон-прямоугольник за текущим словом (эффект маркера) |
| Только слово | Прочие слова скрыты, но сохраняют своё место в строке |
| Зум слова | Активное слово увеличено до 140% |

Все эффекты karaoke совместимы между собой и работают одновременно.  
Стандартные анимации (fade-in, slide-up, …) также можно комбинировать с karaoke-эффектами.

**Сохранить субтитры как SRT** — кнопка **💾 SRT** в заголовке панели субтитров экспортирует все субтитры проекта в файл `.srt` и сохраняет его в разделе История → Субтитры.
...
| Загрузить | Открыть существующий проект из списка |
...
**Поиск проектов и шаблонов** — поле «Поиск…» в боковой панели фильтрует список проектов и список шаблонов по имени в реальном времени.
...
Form (все поля):
  ...
  karaoke_typewriter bool  (false) — побуквенный набор текущего слова
  karaoke_highlight  bool  (false) — цветной фон-маркер за текущим словом
  karaoke_showonly   bool  (false) — скрыть все слова кроме текущего
  karaoke_zoom       bool  (false) — увеличить текущее слово (150%)
...
*Документация актуальна для ветки `IV-12`. Обновлено: 2026-07-28.*
